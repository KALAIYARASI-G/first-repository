...
export const StudentRoster: React.FC = () => {
  ...
  const filteredStudents = students
    .filter(student => {
      ...
      return (
        student.name.toLowerCase().includes(targetSearch) ||
        student.email.toLowerCase().includes(targetSearch) ||
        (student.course && student.course.toLowerCase().includes(targetSearch))
      );
    })
    .sort((a, b) => {
      let comparison = 0;
  ...
  const toggleSort = (field: 'name' | 'course') => {
    if (sortBy === field) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage registration profile data and course tracks connected to PostgreSQL.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex gap-2 justify-end">
          ...
          >
            <ArrowUpDown className="h-3 w-3" /> Sort Name
          </button>
          <button
            onClick={() => toggleSort('course')}
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl border text-xs font-semibold transition-all ${
              sortBy === 'course' ? 'bg-indigo-600/15 border-indigo-500/30 text-indigo-400' : 'bg-slate-900 border-slate-800 text-slate-400'
            }`}
          >
            <ArrowUpDown className="h-3 w-3" /> Sort Course
          </button>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl border border-slate-900 overflow-hidden">
        {loading ? (
          <div className="text-center text-slate-400 py-12">Loading profiles from database...</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-900 text-sm">
                {filteredStudents.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center text-slate-500 py-12">No records found matching search filters.</td>
                  </tr>
                ) : (
                  filteredStudents.map(student => (
                    <tr key={student.id} className="hover:bg-slate-900/20 transition-all">
                      ...
                      <td className="px-6 py-4 font-mono text-xs text-slate-350">{student.id}</td>
                      <td className="px-6 py-4 text-slate-300">{student.phone || 'N/A'}</td>
                      <td className="px-6 py-4 text-slate-350 font-medium">{student.course || 'Unassigned'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="glass w-full max-w-lg rounded-2xl border border-slate-850 overflow-hidden shadow-2xl animate-pulse-slow">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Course / Program Track</label>
                <select
                  value={course}
